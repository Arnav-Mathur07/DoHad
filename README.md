...
The project repository is structured to separate production source files, database schemas, scripting, and research visualization assets:
...
└── image_generation/        # Standalone data analysis outputs (used for papers)
    ├── Arsenic_dosage_infographic.png
    ├── Arsenic_world_map.html
    ├── Cadmium_organ_distribution.png
    ├── organ_pie_chart_grid.py
    └── [Other static maps and visualization scripts...]
```
...
```
  ┌────────────────────────────────────────────────────────┐
  │ 1. PubMed Database Query (Entrez API Search)           │
  └──────────────────────────┬─────────────────────────────┘
                             │
                             ▼
  ┌────────────────────────────────────────────────────────┐
  │ 2. XML Metadata Extraction (Title, Abstract, DOI)      │
  └──────────────────────────┬─────────────────────────────┘
                             │
                             ▼
  ┌────────────────────────────────────────────────────────┐
  │ 3. Automated Preprocessing & Cleaning (Python Scripts)  │
  └──────────────────────────┬─────────────────────────────┘
                             │
            ┌────────────────┴────────────────┐
            ▼                                 ▼
  ┌───────────────────┐             ┌───────────────────┐
  │ 4a. CSV Generation│             │ 4b. SQL Generation│
  │ (Static Ingestion)│             │ (MySQL Seeding)   │
  └───────────────────┘             └───────────────────┘
```

The dataset is maintained using an automated pipeline designed to retrieve, parse, and structure academic literature:
